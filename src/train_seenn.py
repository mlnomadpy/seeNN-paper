...
from utils.load_data_undersample import load_data
...
def train(config):
    ...
    for epoch in range(config.epochs):
        ...
        for steps, (inputs, targets) in enumerate(train_data):
            ...
            if steps == train_data.samples // config.batch_size:
                break
        ...
        for steps, (inputs, targets) in enumerate(val_data):
            ...
            if steps == val_data.samples // config.batch_size:
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_heads', type=int, default=2, help='Key Dim for Multihead Attention')
# apply_weights
